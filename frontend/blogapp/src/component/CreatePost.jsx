  ...
  return (
   ...
   {selectedOption && (
        <Box  m={2} height={'full'}  mt={4}>
          <Image height={'600'} width={'full'} src={getImageUrl()} alt={selectedOption} />
        </Box>
      )}
    <Box m={'auto'} mt={10} width={'70%'}>
      ...
      <form onSubmit={createNewPost}>
        ...
        <FormControl mt={4}>
          ...
          <Input type="text" placeholder="Summary" value={summery} onChange={e => setSummery(e.target.value)} />
        </FormControl>
        <FormControl mt={4}>
          <FormLabel>File</FormLabel>
          <Input type="file" onChange={e => setFile(e.target.files)} />
        </FormControl>
        <ReactQuil value={content} modules={modules} formats={formats}  onChange={newValue => setContent(newValue)}/>
