...
const EditPost = () => {
  ...
  const getImageUrl = () => {
    ...
    useEffect(() => {
      axios
        .get(`http://localhost:8000/postbyid/${id}`)
        .then((res) => {
  ...
  const updatePost = async (e) => {
    ...
    e.preventDefault();
    await axios
      .put("http://localhost:8000/updatepost", data, {
        withCredentials: true,
      })
      .then(() =>alert("Updated post Sucessfully"))
    ...
    <>
      ...
      <Box m={"auto"} mt={10} width={"70%"}>
        ...
        <form onSubmit={updatePost}>
          ...
          />
          <Button width={"full"} mt={7} colorScheme="teal" type="submit">
            Update post
          </Button>
        </form>
      </Box>
